...
export default function App() {
  ...
  const [bgUrl, setBgUrl] = useState<string>('/cerrado_background.png');
  ...
  useEffect(() => {
    const promptVersion = "v6-cerrado-perfect-gal";
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>, type: 'resgate' | 'soltura') => {
    ...
    try {
      ...
      const prompt = type === 'resgate' 
        ? "Extract the GPS coordinates (latitude and longitude) from this image. IMPORTANT: In Brazil, latitude and longitude are NEGATIVE. Ensure the returned values have the correct minus sign and exactly 6 decimal places (e.g., -15.123456). If there is an animal, identify its species (Common Name (Scientific Name)). Para o campo 'circunstancias', atue como um Policial Militar Ambiental realizando o registro de uma ocorrência de resgate de fauna. Analise a imagem e descreva a situação de forma técnica e concisa seguindo as diretrizes: Contexto: O animal foi encontrado em área urbana (local). Estado: Avalie se o espécime apresenta sinais de debilidade/ferimentos ou se está saudável. Procedimento: Descreva o manejo técnico, informando que o animal foi acondicionado em caixa de transporte adequada para garantir a segurança e o bem-estar durante o trajeto. Conclusão: Se o animal estiver debilitado, informe o encaminhamento para assistência veterinária especializada ou centro de reabilitação. Se estiver saudável, informe que o manejo visa a soltura em ambiente seguro. Tom de voz: Formal, objetivo e operacional. Evite repetições de palavras e adjetivos desnecessários. O texto deve ser fluido e direto ao ponto. Return ONLY a JSON object with 'lat', 'lon', 'especie', and 'circunstancias' keys. Use null for any values not found."
